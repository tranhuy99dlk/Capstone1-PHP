...
<?php 
	...
	foreach ($arrNew as $k => $v) {
		...
		for($i=1;$i<$c;$i++) {     //-----------------------------$i must go from 1 when work with qa table
			...
			foreach ($arrDesc as $kDE => $vDE) {
				...
				if (stripos($haystack, $needle) !==false) {
					changeTopic('news','topic','news_id',$arrTO[$i]->id, $v->id, $v->title, $vDE." - ".$arrTO[$i]->title );
					$top=false;
					break;
				}
			}
		...
		if ($top) {
			changeTopic('news','topic','news_id', $arrTO[0]->id, $v->id, $v->title, $arrTO[0]->title );
		}
		...
		foreach ($arrCli as $kCli=> $vCli) {
			...
			foreach ($arrDesc as $kDE => $vDE) {
				...
				$needle = trim(mb_strtolower($vDE));
				if (stripos($haystack, $needle) !==false) {
					changeTopic('news','climate','news_id',$vCli->id,$v->id,$v->title, $vDE." - ".$vCli->title);
					$cli=false;
					break;
				}
			}
		...
		foreach ($arrPa as $kPa=> $vPa) {
			...
			foreach ($arrDesc as $kDE => $vDE) {
				...
				$needle = trim(mb_strtolower($vDE));
				if (stripos($haystack, $needle) !==false) {
					changeTopic('news','patience','news_id',$vPa->id, $v->id,$v->title, $vDE." - ".$vPa->title);
					$pa=false;
					break;
				}
			}
